...
use crate::{Bls12, Fp, Fp12, Fp2, Fp6, G2Prepared, Scalar};
...
mod tests {
    ...
    fn test_gt_generator() {
        assert_eq!(
            Gt::generator(),
            Gt::from(pairing(&G1Affine::generator(), &G2Affine::generator())),
        );
    }
    ...
    fn test_gt_bilinearity() {
        ...
        assert_eq!(
            p,
            Gt::from(pairing(&G1Affine::generator(), &G2Affine::generator())) * c
        );
    }
    ...
    fn test_gt_unitary() {
        ...
        let p = -Gt::from(pairing(&g, &h));
        let q = Gt::from(pairing(&g, &-h));
        let r = Gt::from(pairing(&-g, &h));
