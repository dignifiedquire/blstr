...
use crate::{Fp, Fp12, Fp2, Fp6, G1Affine, G1Projective, G2Affine, G2Projective, Scalar};
...
impl<'de> Deserialize<'de> for Scalar {
    fn deserialize<D: Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        let le_u64s = <[u64; 4]>::deserialize(d)?;
        Scalar::from_u64s_le(&le_u64s).ok_or(D::Error::custom(ERR_CODE))
    }
}
...
impl<'de> Deserialize<'de> for Fp {
    fn deserialize<D: Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        Fp::from_u64s_le(&deser).ok_or(D::Error::custom(ERR_CODE))
    }
}
